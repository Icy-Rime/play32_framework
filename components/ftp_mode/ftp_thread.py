...
#
import usocket, uos, utime, uselect
from micropython import const
import network
import gc

__BUFFER_SIZE = const(4096)
...
def save_file_data(path, dataclient):
    with open(path, "wb") as file:
        chunk = dataclient.recv(__BUFFER_SIZE)
        while len(chunk) > 0:
            file.write(chunk)
            chunk = dataclient.recv(__BUFFER_SIZE)
...
ftp_running = False
ftp_stopped = True
def ftpserver(not_stop_on_quit, addr=None):
    global ftp_stopped    
    DATA_PORT = 13333
    try:
        ftp_stopped = False
        ...
        if addr == None:
            ...
            if wlan.active():
                ...
                if wlan.active():
                    ...
                    ftp_stopped = True
                    return
        ftpsocket = usocket.socket(usocket.AF_INET, usocket.SOCK_STREAM)
        datasocket = usocket.socket(usocket.AF_INET, usocket.SOCK_STREAM)

        ftpsocket.setsockopt(usocket.SOL_SOCKET, usocket.SO_REUSEADDR, 1)
        datasocket.setsockopt(usocket.SOL_SOCKET, usocket.SO_REUSEADDR, 1)

        ftpsocket.bind(usocket.getaddrinfo(addr, 21)[0][4])
        datasocket.bind(usocket.getaddrinfo(addr, DATA_PORT)[0][4])
        ...
        server_poller = uselect.poll()
        data_poller = uselect.poll()
        client_poller = uselect.poll()
        server_poller.register(ftpsocket, uselect.POLLIN)
        data_poller.register(datasocket, uselect.POLLIN)
        print("FTP Server started on ", addr)
        dataclient = None
        fromname = None
        do_run = True
        while do_run and ftp_running:
            s_res = server_poller.poll(100)  # time in milliseconds
            if not s_res:
                continue
            cl, remote_addr = ftpsocket.accept()
            cl.settimeout(300)
            cwd = '/'
            client_poller.register(cl, uselect.POLLIN)
            try:
                # #print("FTP connection from:", remote_addr)
                cl.sendall("220 Hello, this is the ESP8266/ESP32.\r\n")
                while ftp_running:
                    c_res = client_poller.poll(100)  # time in milliseconds
                    if not c_res:
                        continue
                    gc.collect()
                    ...
                    if command == "USER":
                        ...
                        d_res = data_poller.poll(100)  # time in milliseconds
                        if not d_res:
                            continue
                        ...
                        if len(items) >= 6:
                            ...
                            dataclient = usocket.socket(usocket.AF_INET,
                                                       usocket.SOCK_STREAM)
                ...
                client_poller.unregister(cl)
                cl.close()
                cl = None
    finally:
        try:
            server_poller.unregister(ftpsocket)
            data_poller.unregister(datasocket)
            datasocket.close()
            ftpsocket.close()
            if dataclient is not None:
                dataclient.close()
...
def start(addr = None):
    global ftp_running, ftp_stopped
    ftp_running = True
    try:
        import _thread
        _thread.start_new_thread(ftpserver, ((True, addr)))
    except:
        ftp_running = True
        ftpserver(False, addr)
        ftp_stopped = True

def stop():
    global ftp_running
    ftp_running = False
    while not ftp_stopped:
        pass
    pass

def restart(addr = None):
    stop()
    start(addr)
